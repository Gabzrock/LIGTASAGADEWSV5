...
// --- 6. DATA API: Sheetlabs with PapaParse Fallback + AUTOMATION ---
...
function processAWSData(data) {
    // Check if data is different from cached data to prevent unnecessary redraws
    // Simple stringify comparison (works for this scale)
    if (JSON.stringify(data) === JSON.stringify(cachedAWSData)) {
        console.log("No new data updates detected.");
        return; 
    }

    console.log("Updating Map with new AWS Data...");
    cachedAWSData = data; // Update cache
    warningLayerGroup.clearLayers(); // Clear old layers
...
// Function to fetch data (Automated)
function fetchAndRefreshData() {
    ...
    fetch('')
        ...
        .catch(error => { 
            // 2. Failure: Try Google Sheets CSV via PapaParse
...
// Automate Fetch every 60 seconds (60000ms)
